...
# Usage: bash click_virchow2_eval.sh /path/to/target/dir/eval.sh
...
# - linprobe prism wo finetune prism
# - finetune prism
# - linprobe prism after finetune
#
...
# Take eval_block/linprobe_wo_finetune.template and sed MODEL=... with MODEL=prism then append
sed 's/^MODEL=.*/MODEL=prism/' ${SCRIPT_DIR}/eval_block/linprobe_wo_finetune.template >> $TEMP_FILE

# Take eval_block/finetune.template and sed MODEL=... with MODEL=prism then append
sed 's/^MODEL=.*/MODEL=prism/' ${SCRIPT_DIR}/eval_block/finetune.template >> $TEMP_FILE

cat ${SCRIPT_DIR}/eval_block/model_artifact_to_hf_bench.template >> $TEMP_FILE

# Take eval_block/linprobe_after_finetune.template and sed MODEL=... with MODEL=prism then append
sed 's/^MODEL=.*/MODEL=prism/' ${SCRIPT_DIR}/eval_block/linprobe_after_finetune.template >> $TEMP_FILE
