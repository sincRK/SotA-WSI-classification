...
#SBATCH --job-name=dataset_feat_sharded
...
FEAT_DIR="pp_features" # adapt feature dir in $BENCH
PATCH_ENCODER="dino_v3" # adapt patch encoder key as in slurm/feature_extraction/patch_encoder_models/local_ckpts.json
DATA_DIR="${BENCH}/pp" # adapt image dir of dataset
...
bash ${TMPDIR}/feature_extraction/feature_extraction_sharded.sh $DATA_DIR ${TMPDIR}/${FEAT_DIR} 1 18 feat $PATCH_ENCODER 20 512 128 ${TMPDIR}/${FEAT_DIR}/20x_512px_0px_overlap/
rsync -av ${TMPDIR}/${FEAT_DIR} $BENCH

bash ${TMPDIR}/feature_extraction/feature_extraction_sharded.sh $DATA_DIR ${TMPDIR}/${FEAT_DIR} 1 18 feat $PATCH_ENCODER 20 256 128 ${TMPDIR}/${FEAT_DIR}/20x_256px_0px_overlap/
rsync -av ${TMPDIR}/${FEAT_DIR} $BENCH

bash ${TMPDIR}/feature_extraction/feature_extraction_sharded.sh $DATA_DIR ${TMPDIR}/${FEAT_DIR} 1 18 feat $PATCH_ENCODER 10 512 128 ${TMPDIR}/${FEAT_DIR}/10x_512px_0px_overlap/
rsync -av ${TMPDIR}/${FEAT_DIR} $BENCH

bash ${TMPDIR}/feature_extraction/feature_extraction_sharded.sh $DATA_DIR ${TMPDIR}/${FEAT_DIR} 1 18 feat $PATCH_ENCODER 10 256 128 ${TMPDIR}/${FEAT_DIR}/10x_256px_0px_overlap/
rsync -av ${TMPDIR}/${FEAT_DIR} $BENCH
